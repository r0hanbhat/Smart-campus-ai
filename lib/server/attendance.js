...
export async function getTeacherAssignedSubjects(supabase, userId, filters = {}) {
    ...
    const createdSubjects = await listTeacherCreatedSubjects(supabase, userId, filters);
    const mergedSubjects = [...assignedSubjects];

    for (const subject of createdSubjects) {
        if (!mergedSubjects.some((entry) => entry.id === subject.id)) {
...
export async function listTeacherCreatedSubjects(supabase, userId, filters = {}) {
    let query = supabase
        .from('subjects')
...
export async function assertTeacherSubjectAccess(supabase, userId, subjectId) {
    const offerings = await listTeacherCreatedSubjects(supabase, userId);
    const offering = offerings.find((entry) => entry.id === subjectId) || null;
    if (!offering) {
        throw new Error('Teacher access is limited to subjects you created.');
    }
    return offering;
}
