import { CAMPUS_LOCATIONS } from '../smart-campus/constants.ts';
import type { InvalidDateAction, PendingAction } from '../smart-campus/types.ts';
...
export function parseDateFromMessage(dateStr: string) {
...
function createInvalidDateAction(error: InvalidDateAction['error'], message: string): InvalidDateAction {
  return {
    type: 'invalid_date',
    error,
    message,
...
export function detectAction(message: string): PendingAction | InvalidDateAction | null {
  const lowerMsg = message.toLowerCase();

  for (const location of Object.keys(CAMPUS_LOCATIONS)) {
    if (
      lowerMsg.includes(location.toLowerCase()) &&
      (
      ...
      return {
        type: 'navigate',
        destination: location as keyof typeof CAMPUS_LOCATIONS,
    ...
    if (eventMatch) {
      ...
      if (extractedDate) {
        ...
        if (extractedDate < today) {
          return createInvalidDateAction(
            'past_date',
            `The date ${finalDate} is in the past. Please provide a future date.`
          );
        }
      }
      ...
      if (dateStr && !extractedDate) {
        return createInvalidDateAction(
          'invalid_format',
          'I could not understand that reminder date. Please use YYYY-MM-DD.'
        );
      }
  ...
  if (lowerMsg.includes('deadline') || lowerMsg.includes('due date') || lowerMsg.includes('due by')) {
    ...
    if (deadlineMatch) {
      ...
      if (dateStr && !extractedDate) {
        return createInvalidDateAction(
          'invalid_format',
          'I could not understand that deadline date. Please use YYYY-MM-DD.'
        );
      }
...
export function buildCurrentContext(userContext?: UserContext) {
...
export function buildSystemPrompt(params: {
  ...
  const actionPrompt =
    params.action && 'confirmation' in params.action
