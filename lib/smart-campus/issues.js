...
export function normalizeIssueNotificationPreferences(preferences) {
    return {
...
export function createIssueRecord({ title, description, category, priority, location, evidence, reporter, notificationPreferences }) {
    ...
    return {
        ...
        notificationPreferences: normalizeIssueNotificationPreferences(notificationPreferences),
        satisfaction: {
            rating: null,
            comment: '',
            submittedAt: '',
...
export function normalizeIssueRecord(issue) {
    ...
    return {
        ...
        notificationPreferences: normalizeIssueNotificationPreferences(issue.notificationPreferences),
        satisfaction: {
...
export function normalizeIssueCenter(center) {
    return {
        ...
        notificationPreferences: normalizeIssueNotificationPreferences(center?.notificationPreferences),
    };
}
...
export function applyAdminIssueUpdate(issue, { updates = {}, note = '', actorName = 'Admin', actorRole = 'admin' }) {
    ...
    const resolutionSummaryChanged = resolutionSummary !== currentIssue.resolutionSummary;
    ...
    const afterEvidenceChanged = JSON.stringify(afterEvidence) !== JSON.stringify(currentIssue.beforeAfter.after);
    ...
    else if (resolutionSummaryChanged || afterEvidenceChanged) {
        nextTimeline.unshift(createIssueTimelineEntry({
            status: nextStatus,
            title: resolutionSummaryChanged ? 'Resolution update added' : 'After photos updated',
            notes: resolutionSummaryChanged
                ? resolutionSummary
                : 'Admin uploaded updated completion evidence.',
            actorName,
            actorRole,
            createdAt: now,
        }));
