...
create table if not exists public.notifications (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles(user_id) on delete cascade,
  type text not null check (type in ('friend_request', 'friend_accept', 'group_invite', 'teacher_announcement')),
  title text not null,
  body text not null,
...
begin
  ...
  -- Check if requestor is already a member
  select exists (
    select 1
    from public.conversation_members
    where conversation_id = p_conversation_id
      and user_id = auth.uid()
  ) into v_is_requestor_member;

  -- Allow if:
  -- 1. Adding yourself to a group
  -- 2. You're already a member and it's a group
  if v_conversation_type != 'group' then
    return jsonb_build_object('ok', false, 'error', 'Can only add members to groups');
  end if;

  if p_user_id != auth.uid() and not v_is_requestor_member then
    return jsonb_build_object('ok', false, 'error', 'You must be a member to add others');
  end if;

  -- Insert the member (on conflict do nothing to handle duplicates)
  insert into public.conversation_members (conversation_id, user_id)
